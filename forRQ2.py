...
class DistributionAnalyzer:
    ...
    def analyze_impact(self, X_train: np.ndarray, X_test: np.ndarray, y_train: np.ndarray,
                       ...
                       use_hyper_opt: bool = False,  # 新增：是否启用超参数优化
                       cv: int = 5,  # 新增：交叉验证折数
    ...
    def optimize_training_set_selection(self, X_pool: np.ndarray, y_pool: np.ndarray,
        ...
        if method == "random":
            ...
            if use_approximate:
                # 使用近似最近邻加速（仅支持部分度量）
                supported_metrics = ['euclidean', 'jsd', 'weighted_jsd', 'minkowski']  # 新增jsd支持
                ...
                if distance_metric == "euclidean":
                    ...
                    X_pool_dist = normalize_to_distribution(X_pool_transformed)
                    X_test_dist = normalize_to_distribution(X_test_transformed)
                    ...
                    # 计算原始KL散度：KL(P||M) = sum(P_i * log(P_i/M_i))
...
def main():
    ...
    X_pool = np.vstack([X_res, X_test])
    y_pool = np.hstack([y_res, y_test])
    # X_pool = X_res
    # y_pool = y_res
    ...
    print(f"\nJS散度筛选后能量距离：{ed:.4f} MMD: {mmd:.4f}  JSD: {jsd:.4f} 加权JSD：{weighted_jsd:.4f}")
    # 使用优化后的训练集重新评估模型性能
...
data_path = r"D:\projects\pythonProject\source_data\PROMISE-backup-master\PROMISE-backup-master\bug-data\jedit\jedit-3.2.csv"
optimal_weights = np.array(
    [0.0104,0.2022,0.0375,0.0161,0.0219,0.0337,0.0144,0.0030,0.0033,0.0059,0.0108,0.0556,0.0611,0.0233,0.2757, 0.1081,0.0464,0.0172,0.0291,0.0244])
